...
# from sglang_lib.schedule_batch import (
#     FINISH_ABORT,
#     MultimodalInputs,
#     Req,
#     ScheduleBatch,
...
# from sglang_lib.model_router import ModelRouter
...
class Scheduler(
    ...
    def __init__(
        ...
        server_args_list: Optional[List[ServerArgs]] = None,
        router_args: Optional[Dict] = None,
    ):
        # Parse args
        ...
        # print(f"enable_overlap: {self.enable_overlap}")
        ...
        #     # from sglang.srt.speculative.eagle_worker import EAGLEWorker
        #     from sglang_lib.eagle_worker import EAGLEWorker
        ...
        #     #     dp_rank=dp_rank,
        #     # )
        #     self.eagle_worker = TpWorkerClass(
        #             server_args=server_args,
        #             gpu_id=gpu_id,
        #             tp_rank=tp_rank,
        #             pp_rank=pp_rank,
        ...
        #             nccl_port=port_args.nccl_port,
        #         )
        #     model_ids = [server_args.speculative_draft_model_path, server_args.model_path]

        #     # self.spec_router = ModelRouter(model_ids)
        #     self.model_hub = ModelHub(workers=[self.eagle_worker, self.tp_worker], orchestration_strategy="speculative")


        # if len(server_args_list) > 1:
        #     # from sglang.srt.speculative.eagle_worker import EAGLEWorker
        #     # self.draft_worker = EAGLEWorker(
        #     #     gpu_id=gpu_id,
        #     #     tp_rank=tp_rank,
        ...
        #     }
        #     self.model_hub = ModelHub(server_args_list=server_args_list, 
        #                               router_args=router_args, 
        #                               orchestration_strategy="speculative", 
        #                               main_worker_kargs=main_worker_kargs,
        #                               spec_flag=self.spec_flag)
        #     self.tp_worker = self.model_hub.get_worker(-1)
        #     print("target_worker", TpWorkerClass, vars(self.tp_worker))
        # else:
        ...
        self.tp_worker = self.model_hub.get_worker(-1)
        # print("target_worker", vars(self.tp_worker))
        ...
        if global_server_args_dict["max_micro_batch_size"] is None:
            ...
            if hasattr(self.model_hub, 'mode_switch_enabled') and self.model_hub.mode_switch_enabled:
                # 动态切换模式：根据当前模式设置
                effective_max_requests = self.model_hub.get_effective_max_running_requests()
                global_server_args_dict["max_micro_batch_size"] = max(
                    effective_max_requests // server_args.pp_size, 1
                )
                logger.info(f"Dynamic mode: initial max_micro_batch_size={global_server_args_dict['max_micro_batch_size']} "
                           f"for mode {self.model_hub.current_mode}")
            else:
                # 传统模式
                global_server_args_dict["max_micro_batch_size"] = max(
    ...
    def event_loop_normal(self):
        ...
        while True:
            ...
            # if batch:
            #     logger.info(f"batch, before run batch{batch.spec_info}, bs {batch.batch_size()}")
    ...
    def log_decode_stats(
        ...
        forward_iter = self.model_hub.speculative_num_steps if self.spec_flag else 1
        ...
        msg = (
            ...
            f"#token: {num_used}, "
            f"token usage: {num_used / self.max_total_num_tokens:.2f}, "
            f"gap latency: {gap_latency / (self.server_args.decode_log_interval*forward_iter):.2f}, "
    ...
    def prepare_dp_attn_batch_raw(
        ...
        if local_batch is None:
            ...
            # if spec_flag and spec_algorithm.is_eagle():
            if spec_flag:
    ...
    def flush_cache(self):
        ...
        if (
            ...
            if self.enable_spec():
                ...
                self.model_hub.memory_pool_clear()
    ...
    def get_internal_state(self, recv_req: GetInternalStateReq):
        ...
        if self.enable_spec() and self.cum_spec_accept_count > 0:
...
def run_scheduler_process(
    ...
    try:
        scheduler = Scheduler(server_args, port_args, gpu_id, tp_rank, pp_rank, dp_rank, server_args_list=server_args_list, router_args=router_args)
        pipe_writer.send(
            {
                "status": "ready",
