...
class Runtime:
    ...
    def __init__(
        self,
        log_level: str = "error",
        server_configs_list: Optional[List] = None,
        ...
        **kwargs,
        ...
        # from sglang_lib.http_server import launch_server
        # from sglang_lib.server_args import ServerArgs
        from sglang.srt.entrypoints.http_server import launch_server
        from sglang.srt.server_args import ServerArgs
        from sglang.srt.utils import is_port_available

        print(server_configs_list, "server_configs_list")

        self.router_args = router_args

        if server_configs_list is not None:
            self.server_args_list = []
            for server_args_config in server_configs_list:
                if isinstance(server_args_config, dict):
                    # 如果是字典，转换为ServerArgs对象
                    server_args = ServerArgs(log_level=log_level, **server_args_config)
                elif isinstance(server_args_config, ServerArgs):
                    # 如果已经是ServerArgs对象，直接使用
                    server_args = server_args_config
                else:
                    raise ValueError(f"Invalid server_args_config type: {type(server_args_config)}")
                
                self.server_args_list.append(server_args)
            self.server_args = self.server_args_list[-1]
        else:
            self.server_args = ServerArgs(*args, log_level=log_level, **kwargs)
            self.server_args_list = [self.server_args]
        ...
        proc = multiprocessing.Process(
            ...
            kwargs={
                "server_args_list": self.server_args_list,
