...
class Gemma4Attention(nn.Module):
    def __init__(
        ...
        self._fa_max_head_dim = 256
        self._need_split_attn = self.head_dim > self._fa_max_head_dim
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self._need_split_attn:
            # head_dim > 256: FA doesn't support this. Use manual KV cache + PyTorch SDPA.
            # 1) Save KV to cache
            if k is not None and not self.is_kv_shared_layer:
                cache_loc = forward_batch.out_cache_loc
                forward_batch.token_to_kv_pool.set_kv_buffer(
                    self.attn, cache_loc, k, v
                )
            # 2) Gather full KV from cache for all tokens in the batch
            k_cache = forward_batch.token_to_kv_pool.get_key_buffer(self.attn.layer_id)
            v_cache = forward_batch.token_to_kv_pool.get_value_buffer(self.attn.layer_id)
            # k_cache/v_cache shape: (pool_size, kv_heads, head_dim)
            # Gather by req_pool_indices + seq_lens to build per-request KV
            # For simplicity use a batched SDPA with padding
            req_pool_indices = forward_batch.req_pool_indices
            seq_lens = forward_batch.seq_lens
            req_to_token = forward_batch.req_to_token_pool
            bs = len(seq_lens)
            max_len = max(seq_lens) if len(seq_lens) > 0 else 1
            device = q.device
            dtype = q.dtype

            # Build padded K, V tensors
            k_padded = torch.zeros(bs, max_len, self.num_kv_heads, self.head_dim, device=device, dtype=dtype)
            v_padded = torch.zeros(bs, max_len, self.num_kv_heads, self.head_dim, device=device, dtype=dtype)
            # Build attention mask
            attn_mask = torch.full((bs, max_len), float('-inf'), device=device, dtype=dtype)
            for i in range(bs):
                sl = seq_lens[i]
                token_indices = req_to_token.req_to_token[req_pool_indices[i], :sl]
                k_padded[i, :sl] = k_cache[token_indices]
                v_padded[i, :sl] = v_cache[token_indices]
                attn_mask[i, :sl] = 0.0

            # q shape: (total_tokens, num_heads, head_dim) -> need per-request
            # For extend mode, q has variable length per request
            q_lens = forward_batch.extend_seq_lens if hasattr(forward_batch, 'extend_seq_lens') and forward_batch.extend_seq_lens is not None else [q.shape[0]]

            # Simple fallback: process each request individually
            outputs = []
            q_offset = 0
            for i in range(bs):
                ql = q_lens[i] if i < len(q_lens) else q.shape[0] - q_offset
                qi = q[q_offset:q_offset+ql]  # (ql, num_heads, head_dim)
                qi = qi.unsqueeze(0).transpose(1, 2)  # (1, num_heads, ql, head_dim)
                ki = k_padded[i:i+1, :seq_lens[i]].transpose(1, 2)  # (1, kv_heads, sl, head_dim)
                vi = v_padded[i:i+1, :seq_lens[i]].transpose(1, 2)
                oi = torch.nn.functional.scaled_dot_product_attention(
                    qi, ki, vi, is_causal=bool(ql > 1), enable_gqa=True,
                    scale=self.attn.scaling if hasattr(self.attn, 'scaling') else None,
                )
                oi = oi.transpose(1, 2).squeeze(0)  # (ql, num_heads, head_dim)
                outputs.append(oi)
                q_offset += ql
        ...
        if self._need_split_attn and attn_output.dim() == 3:
            pass  # already in (tokens, heads, head_dim) format
...
class Gemma4ForCausalLM(PreTrainedModel):
    ...
    def __init__(
        ...
        # When loaded via Gemma4ForConditionalGeneration alias, config is the
        # top-level Gemma4Config; unwrap to text_config for the causal LM.
            ...
            for param_name, weight_name, shard_ids in expert_params_mapping:
                ...
                for param_name, weight_name, shard_id in stacked_params_mapping:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"[EAGLE3_DEBUG] skip weight {name}: "
                            f"param={param.shape} loaded={loaded_weight.shape} err={e}"
                        )
                        continue
...
# Text-only alias: lets the registry resolve Gemma4ForConditionalGeneration
# (the architecture listed in HF configs) to the causal-only implementation,
# skipping vision/audio towers.
