...
# from sglang_lib.server_args import ServerArgs
...
class ModelRunner:
    ...
    def __init__(
        ...
        self.pending_memory_init = None
        # Parse args
        self.model_config = model_config
        self.mem_fraction_static = mem_fraction_static
        self.device = server_args.device
        self.gpu_id = gpu_id
        self.defer_memory_init = defer_memory_init
        ...
        if not defer_memory_init:
            self.init_memory_and_backend(min_per_gpu_memory, req_to_token_pool, token_to_kv_pool_allocator)
        else:
    ...
    def init_memory_and_backend(self, min_per_gpu_memory: float, 
    ...
    def init_torch_distributed(self):
        ...
        logger.info(f"min_per_gpu_memory: {min_per_gpu_memory}, local_gpu_memory: {local_gpu_memory}")
    ...
    def profile_max_num_token(self, total_gpu_memory: int):
        print("self.device, self.gpu_id", self.device, self.gpu_id)
    ...
    def profile_rest_memory(self, total_gpu_memory: int):
        available_gpu_memory = get_available_gpu_memory(
            self.device,
            self.gpu_id,
            distributed=get_world_group().world_size > 1,
    ...
    def calc_max_num_for_aggressive_mode(self, total_gpu_memory: int, max_num_reqs: Optional[int] = None, max_total_tokens: Optional[int] = None):
    ...
    def get_model_kvcache_info(self):
        if self.is_draft_worker:
            num_layers = getattr(
                self.model_config.hf_config,
                "num_nextn_predict_layers",
    ...
    def calc_max_num_for_speculative_mode(self, total_gpu_memory: int, 
                                          visible_gpu_memory: float,
                                          max_num_reqs: Optional[int] = None, 
                                          max_total_tokens: Optional[int] = None):
        # self.max_total_num_tokens = self.profile_max_num_token(total_gpu_memory)
        num_layers, cell_size = self.get_model_kvcache_info()
        # 求出剩余内存容量
        # available_gpu_memory = get_available_gpu_memory(
        #     self.device,
        #     self.gpu_id,
        ...
        #     1 - self.mem_fraction_static
        # )
        max_num_cell = int(visible_gpu_memory * (1 << 30) // cell_size)
        ...
        if self.is_draft_worker:
            logger.info(f"draft_runner_cache_size: {self.server_args.draft_runner_cache_size}, max_num_reqs: {self.server_args.max_num_reqs}")
            self.max_total_num_tokens = self.server_args.draft_runner_cache_size
            # self.max_total_num_tokens = self.server_args.max_total_num_tokens_target
            ...
            # )
            if max_num_reqs is None:
                max_num_reqs = min(int(self.max_total_num_tokens * self.model_config.context_len / 512), 4096)

            # self.server_args.max_total_num_tokens_target = self.max_total_num_tokens_target
            # Target worker and draft worker shares the same indices for the
            # token_to_kv_pool, so we should make sure to match max_total_num_tokens.
            self.server_args.draft_runner_cache_size = self.max_total_num_tokens
            self.server_args.max_num_reqs = max_num_reqs
            logger.info(f"max_total_num_tokens: {self.max_total_num_tokens}, max_num_reqs: {max_num_reqs}, draft_runner_cache_size: {self.server_args.draft_runner_cache_size}")
    ...
    def init_memory_pool(
        ...
        if self.enable_spec():
            self.max_total_num_tokens, max_num_reqs = self.calc_max_num_for_speculative_mode(total_gpu_memory, visible_gpu_memory, max_num_reqs, max_total_tokens)
        else:
            self.max_total_num_tokens, max_num_reqs = self.calc_max_num_for_aggressive_mode(total_gpu_memory, max_num_reqs, max_total_tokens)
        ...
        if self.token_to_kv_pool_allocator is None:
            ...
            # Draft worker shares token_to_kv_pool_allocator with the target worker or model hub.
            assert self.is_draft_worker or self.token_to_kv_pool_allocator.is_from_model_hub
    ...
    def _forward_raw(
        ...
        if can_run_cuda_graph:
            ...
            # print(f"model_runner forward_extend ret: {ret}")
